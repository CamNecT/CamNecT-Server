...
public class GifticonController {
    ...
    @Operation(summary = "기프티콘 샵 홈(상품목록+내포인트)", description = "서버 캐시 DB의 상품목록, 사용자 포인트, 기본 수신 전화번호·이메일을 한번에 내려줍니다.")
    @ApiResponses({
    ...
    @Operation(summary = "구매 확정", description = "포인트 차감과 구매요청 저장 후 관리자가 엑셀의 수신 전화번호로 구매·발송하며 이메일도 함께 전달합니다. recipientPhone을 생략하면 가입 전화번호를, recipientEmail을 생략하면 가입 이메일을 사용합니다.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "요청 성공", useReturnTypeSchema = true),
